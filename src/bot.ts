...
async function statusTask() {
    ...
    while (1) {
        ...
        client.user?.setPresence({ activities: [{ name: "sudo apt-get currypan", url: "https://256server.com/", type: Discord.ActivityType.Streaming }] });
        await sleep(5000);
    }
}
...
client.on("messageCreate", async (message) => {
    ...
    if (message.author.bot || !message.member || !message.guild || !message.content.split(" ")[0].startsWith((env.BOT_PREFIX as string))) return;
    const [cmd, ...args] = message.content.slice((env.BOT_PREFIX as string).length).replace("　", " ").split(" ").filter(v => v != "");
